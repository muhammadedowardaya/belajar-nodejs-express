...
const addPoweredHeader = (req, res, next) => {
	res.set("X-Powered-By", "Muhammad Edo Wardaya");
	next();
};
...
test("Test Response Middleware", async () => {
	...
	expect(response.get("X-Powered-By")).toBe("Muhammad Edo Wardaya");
	expect(response.text).toBe("Hello Response");
});

test("Test Response Middleware 2", async () => {
	const response = await request(app).get("/edo").query({ apiKey: "123" });
	expect(response.get("X-Powered-By")).toBe("Muhammad Edo Wardaya");
	expect(response.text).toBe("Hello Edo");
});

test("Test Response Middleware Unauthorized", async () => {
	const response = await request(app).get("/edo");
